...
% --- Contatori indipendenti per tipo ---
\newcounter{lstSQL}
\newcounter{lstPy}
\newcounter{lstTxt}
...
\lstnewenvironment{sqllisting}[1][]{%
  ...
  \setcounter{lstlisting}{\value{lstSQL}}%
  \lstset{#1}%
}{%
  \setcounter{lstSQL}{\value{lstlisting}}%
}

\lstnewenvironment{pylisting}[1][]{%
  \renewcommand{\lstlistingname}{Codice}%
  \setcounter{lstlisting}{\value{lstPy}}%
  \lstset{style=pystyle,#1}%
}{%
  \setcounter{lstPy}{\value{lstlisting}}%
}

\lstnewenvironment{txtlisting}[1][]{%
  \renewcommand{\lstlistingname}{Prompt}%
  \setcounter{lstlisting}{\value{lstTxt}}%
  \lstset{style=txtstyle,#1}%
}{%
  \setcounter{lstTxt}{\value{lstlisting}}%
}
...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item Streamlit per una dashboard molto semplice da implentare, poiché la parte di HTML, CSS e Javascript è del tutto trasparente al programmatore, garantendo delle pagine Web semplici e soprattutto intuitive.
    \end{itemize}
    \section{Dataset di riferimento ed ingestione dei dati}   
    ...
    \begin{pylisting}[style=pystyle, caption={Ingestione CSV e conversione in Parquet}, label=lst:ingest]
    spark = None
    ...
    \begin{itemize}
        ...
        \item dei buffer circolari a dimensioni fissa;
    ...
    \begin{itemize}
        \item timestamp, cioè la data e ora;
        \item summary, che è la descrizione tramite scapy;
        \item length, ossia la dimensione del pacchetto;
        \item src, ovvero l'IP della sorgente;
        \item dst, cioè l'IP del destinatario;
        \item proto, che è il protocollo di rete utilizzato.
    \end{itemize}
    ...
    le interrogazioni richiedono delle CTE, raggruppamenti, anche case, esprimibili più semplicemente in linguaggio
    SQL piuttosto che in Python o in Java. Tuttavia, tale scelta è del tutto soggettiva.
    \subsection{Analisi mitigazione}
    L'analisi mitigazione consiste nell'eseguire una query che prende i cinque attaccanti con maggior numero di occorenze,
    raggruppando per indirizzo IP sorgente e tipologia di traffico malevolo e 
    ...
    complicato.
    \begin{sqllisting}[caption={Analisi del traffico malevolo per indirizzo IP}, label=lst:analisi_traffico]
    with traffico as (
    ...
    router che si interfaccia nel data lake.
    \begin{figure}[H]
        \centering
    ...
    \end{figure}
    \noindent Il pannello riporta inoltre il numero di combinazioni distinte indirizzo IP--tipologia di attacco individuate dalla CTE (15 nel caso in esame): un valore
    contenuto, tipico di un ambiente di test con pochi nodi attaccanti simulati, che si distingue dal conteggio dei singoli flussi malevoli (nell'ordine dei milioni)
    riportato nella tabella di dettaglio.\\
    In questa query viene eseguita la cosiddetta analisi delle performance, in cui viene confrontato la scalabilità in Spark e con un DB relazionale.
    In particolare, il tempo del DB relazionale non è misurato, ma stimato: si parte dal tempo reale dell'analisi Spark sui 66 milioni di record. In particolare:
    \begin{itemize}
    ...
    \begin{sqllisting}[caption={Bilanciamento del dataset}, label=lst:bilanciamento_dataset]
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{figure/bilanciamento.png}
        \caption{Dashboard che riporta il bilanciamento del dataset}
        \label{fig:bilanciamento}
    \end{figure}
    \noindent Dai risultati emerge uno sbilanciamento marcato su una scala di decine di milioni di record: il traffico Benign copre circa il 55\% dei flussi
    e il DoS un altro 12\%, mentre la maggior parte delle restanti classi (es. Worms, Shellcode, Analysis) resta sotto lo 0.1\%. Su un volume di questa
    scala il ribilanciamento non può essere affrontato con un campionamento in memoria: richiede strategie distribuite (undersampling/oversampling
    partizionato, pesi di classe) applicabili nativamente con Spark MLlib, confermando l'utilità dell'elaborazione distribuita già in questa fase
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{figure/homepage.png}
        \caption{Homepage della dashboard con tutto funzionante}
        \label{fig:homepage}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{figure/attacco_corso.png}
    ...
    \begin{sqllisting}[caption={Andamento temporale}, label=lst:andamento_temporale]
        select 
    ...
    \begin{figure}[H]
        ...
        \label{fig:tempo}
    \end{figure}
    \noindent Il grafico evidenzia un picco isolato che supera i 2 miliardi di byte in una singola finestra da un minuto, a fronte di un andamento pressoché
    piatto nel resto del periodo. Aggregare oltre 66 milioni di flussi in finestre temporali ed individuare simili picchi in un'unica passata è esattamente
    il tipo di calcolo che beneficia della distribuzione del carico su più executor Spark, difficilmente sostenibile con un'aggregazione single-node
    sull'intero storico. Si noti che l'asse temporale riflette i timestamp originali del dataset storico e non
    l'orario della simulazione live.
    ...
    \begin{sqllisting}[caption={Distribuzione dei protocolli e flussi}, label=lst:porte]
        select 
            l4_dst_port as port,
            case 
    ...
    \begin{figure}[H]
        ...
        \label{fig:porte}
    \end{figure}
    \noindent Dal grafico risulta che le porte 80 e 21 concentrano la quota nettamente maggiore dei flussi, a fronte di una lunga coda di porte con volumi
    residuali. Dal punto di vista dell'elaborazione, la query raggruppa 66 milioni di flussi su tre dimensioni (porta, protocollo, label) riducendoli a sole
    50 righe aggregate: un'operazione che sfrutta il group-by distribuito di Spark su un volume che renderebbe proibitivo un equivalente calcolato
    single-node.
    ...
    \begin{sqllisting}[caption={Top talkers}, label=lst:top_talkers]
        select 
            ipv4_src_addr as source_ip, ipv4_dst_addr as destination_ip, label,
            sum(in_bytes + out_bytes) as total_bytes, count(*) as flow_count
        from traffico_nids
    ...
    \begin{figure}[H]
        ...
        \label{fig:heatmap}
    \end{figure}
    \noindent La heatmap evidenzia poche coppie sorgente-destinazione responsabili della quasi totalità del volume in byte, con la scala colore che arriva
    fino a 10 miliardi di byte per singola coppia: un pattern che a livello di elaborazione richiede di ordinare per volume aggregato l'intero
    prodotto cartesiano IP sorgente/destinazione osservato nei 66 milioni di flussi, operazione per cui il piano di shuffle distribuito di Spark SQL è
    determinante rispetto a un join/order-by eseguito su una singola macchina.
    ...
    caratteristiche dell'hardware del calcolatore usato. Infatti, siccome i parametri degli LLM sono nell'ordine dei miliardi, il carico è stato diviso tra RAM (16 GB) e VRAM (8 GB),
    per evitare rallentamenti e soprattutto crash per out of memory.
    ...
    \begin{figure}[H]
        ...
        \label{fig:llm}
    \end{figure}
    \begin{txtlisting}[style=txtstyle, caption={Invocazione di Llama}, label=lst:llama]
    ...
    \end{txtlisting}
    \begin{txtlisting}[style=txtstyle, caption={Invocazione di Qwen}, label=lst:qwen]
        Sei un assistente virtuale esperto di cybersecurity per il Data Center.
    ...
    \begin{figure}[H]
        ...
        \label{fig:query_llm}
    \end{figure}
    \begin{txtlisting}[style=txtstyle, caption={Invocazione di Llama per generare la query}, label=lst:query_llama]
    ...
    \end{txtlisting}
    \begin{txtlisting}[style=txtstyle, caption={Invocazione di Qwen per generare la query}, label=lst:query_qwen]
        Sei un assistente esperto in Data Engineering, specializzato nella scrittura di query SQL per Apache Spark.
    ...
    \subsection{Catalogo}
    Il catalogo implementa un metadata store centralizzato su MongoDB che censisce tutte le fonti dati del sistema (dataset
    storico NIDS, traffico live, modelli LLM), mostrando per ciascun schema, stato online/offline, formato e 
    ...
    \begin{itemize}
        \item ingestion eterogenea: dataset storico Parquet (66 milioni+ record) e dati live PCAP, entrambi interrogabili via
